...
var utm_source_sm_fc = getCookie('utm_source'), utm_medium_sm_fc = getCookie('utm_medium'), utm_campaign_sm_fc = getCookie('utm_campaign'), utm_content_sm_fc = getCookie('utm_content'), utm_term_sm_fc = getCookie('utm_term');
  ...
  if (utm_arr[i].toLowerCase().search('utm_source')>=0) {var utm_source_sm = utm_arr[i].split('=').pop(); utm_arr[i] = ''} ;
  if (utm_arr[i].toLowerCase().search('utm_medium')>=0) {var utm_medium_sm = utm_arr[i].split('=').pop(); utm_arr[i] = ''} ;
  if (utm_arr[i].toLowerCase().search('utm_campaign')>=0) {var utm_campaign_sm = utm_arr[i].split('=').pop(); utm_arr[i] = ''} ;
  if (utm_arr[i].toLowerCase().search('utm_content')>=0) {var utm_content_sm = utm_arr[i].split('=').pop(); utm_arr[i] = ''} ;
  if (utm_arr[i].toLowerCase().search('utm_term')>=0) {var utm_term_sm = '-_-'+utm_arr[i].split('=').pop(); utm_arr[i] = ''} ;

	}
	
    if (utm_source_sm == '') { utm_source_sm = utm_source_sm_fc };
    if (utm_medium_sm == '') { utm_medium_sm = utm_medium_sm_fc };
    if (utm_campaign_sm == '') { utm_campaign_sm = utm_campaign_sm_fc };
    if (utm_content_sm == '') { utm_content_sm = '-_-'+utm_content_sm_fc.replace(/[^a-zA-Z0-9_=]/g, '')};
    if (utm_term_sm == '') { utm_term_sm = '-_-'+utm_term_sm_fc.replace(/[^a-zA-Z0-9_=]/g, '')};
